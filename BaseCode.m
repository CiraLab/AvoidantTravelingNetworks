...
% clc
% clear
seed_num = 3; %input('Please enter the seed number: '); %if you want to reproduce same results set a seed
rng(seed_num);
...
for t=1:ceil(runtime*dt)
    ...
    if rem(t-1,display)==0
        ...
        frame = getframe(gcf);
        frame.cdata = imresize(frame.cdata, [460 520]);
...
if display ~= 0
    title_str = strcat("Runtime: ", num2str(runtime));
    title_str2 = strcat("kG: ", num2str(kG), " kB: ", num2str(kB), " kR: ", num2str(kR), " kS: ", num2str(kS));
